...
#SBATCH --mem=256G
#SBATCH --output=PENgeom-time-mistral:7b-%j.out
...
start_ollama_gpu() {
    ...
    $OLLAMA_BIN serve > mistral:7bgeom_time_ollama_gpu${gpu_id}.log 2>&1 &
    ...
    if ! kill -0 $pid 2>/dev/null; then
        ...
        tail -20 mistral:7bgeom_time_ollama_gpu${gpu_id}.log
        return 1
    fi
    ...
    if [ "$is_master" = "true" ]; then
        ...
        while true; do
            ...
            if ! kill -0 $pid 2>/dev/null; then
                ...
                echo "📜 Ultimi log:"
                tail -30 mistral:7bgeom_time_ollama_gpu${gpu_id}.log
                return 1
            fi
            ...
            if [ $((attempts % 10)) -eq 0 ]; then
                ...
                local progress=$(grep "model load progress" mistral:7bgeom_time_ollama_gpu${gpu_id}.log | tail -1)
...
if [ $WORKING_GPUS -eq 0 ]; then
    ...
    for i in 0 1 2 3; do
        ...
        tail -30 mistral:7bgeom_time_ollama_gpu${i}.log 2>/dev/null || echo "Log non disponibile"
    done
    exit 1
fi
...
advanced_gpu_monitor() {
    ...
    while true; do
        ...
        if pgrep -f "veronacard_mob_with_geom_time_copy_copy" >/dev/null; then
            ...
            if [ -f "mistral:7b_geom_time_python_execution.log" ]; then
                processed=$(grep -c "Processing card" mistral:7b_geom_time_python_execution.log 2>/dev/null || echo "0")
                errors=$(grep -c "ERROR\|Error" mistral:7b_geom_time_python_execution.log 2>/dev/null || echo "0")
...
if [ -f "data/verona/vc_site.csv" ]; then
    python3 -u veronacard_mob_with_geom_time_copy_copy.py \
        --append 2>&1 | tee mistral:7b_geom_time_python_execution.log
    PYTHON_EXIT=$?
else
