...
class Config:
    ...
    RESULTS_DIR = Path(__file__).resolve().parent / "results/middle/llama3.1_8b/with_geom_time_cluster/"
...
class PromptBuilder:
    ...
    def create_prompt(
        ...
        cluster_ranking = " > ".join(cluster_top_pois[:5])

        # Create enhanced prompt with temporal context and cluster preferences
        return f"""
            Tourist cluster {cluster_id} (preference: {cluster_ranking}).
