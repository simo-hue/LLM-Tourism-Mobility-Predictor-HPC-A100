...
start_ollama_gpu() {
    ...
    if ! kill -0 $pid 2>/dev/null; then
        ...
        tail -20 mixgeom_time_ollama_gpu${gpu_id}.log
        return 1
    fi
    ...
    if [ "$is_master" = "true" ]; then
        ...
        while true; do
            ...
            if ! kill -0 $pid 2>/dev/null; then
                ...
                echo "📜 Ultimi log:"
                tail -30 mixgeom_time_ollama_gpu${gpu_id}.log
                return 1
            fi
            ...
            if [ $((attempts % 10)) -eq 0 ]; then
                ...
                local progress=$(grep "model load progress" mixgeom_time_ollama_gpu${gpu_id}.log | tail -1)
...
if [ $WORKING_GPUS -eq 0 ]; then
    ...
    for i in 0 1 2 3; do
        ...
        tail -30 mixgeom_time_ollama_gpu${i}.log 2>/dev/null || echo "Log non disponibile"
    done
    exit 1
fi
