...
class Config:
    ...
    # 🔧 OLLAMA Model configuration - Sequential execution with Mistral 7B
    MODEL_NAME: str = "mistral:7b"  # Ollama format
    FALLBACK_MODELS = ["qwen2.5:7b", "llama3.1:8b"]
    POI_PREDICTIONS = 5  # Number of POI predictions
    ...
    # 🔧 OLLAMA Sequential configuration - optimized for stability
    MAX_CONCURRENT_REQUESTS = 1  # 🔧 SEQUENTIAL: Only 1 request at a time
    MAX_CONCURRENT_PER_GPU = 1   # 🔧 SEQUENTIAL: 1 request per GPU
    REQUEST_TIMEOUT = 1200       # 🔧 20 min timeout for temporal complexity
    CIRCUIT_BREAKER_THRESHOLD = 50  # 🔧 Failure threshold

    # 🔧 Retry configuration
    MAX_RETRIES_PER_REQUEST = 5  # 🔧 Maximum retries per request
    MAX_503_RETRIES = 3          # 🔧 Max retries for 503 errors
    RETRY_ON_503_WAIT = 2        # 🔧 Wait seconds on 503 error
    BACKOFF_BASE = 2             # 🔧 Base backoff multiplier
    BACKOFF_MAX = 60             # 🔧 Maximum backoff time

    # 🚀 OLLAMA Generation parameters - MAXIMUM GPU UTILIZATION for Mistral 7B
    TEMPERATURE = 0.1  # 🔧 Fast deterministic generation
    MAX_TOKENS = 64    # 🔧 Reasonable response length
    TOP_P = 0.9
    TOP_K = 10         # 🔧 Ollama sampling parameter
    NUM_CTX = 2048     # 🚀 INCREASED: Larger context window for GPU
    NUM_BATCH = 512    # 🚀 DOUBLED: Larger batch for GPU throughput
    NUM_THREAD = 112   # 🚀 MAXIMUM: All CPU cores for preprocessing

    # 🔧 OLLAMA Connection settings
    OLLAMA_PORT_FILE = "ollama_ports.txt"

    # 🚀 GPU MAXIMIZATION parameters - Force all layers on GPU
    GPU_LAYERS = -1              # 🚀 ALL LAYERS: -1 = all layers on GPU
    GPU_MEMORY_UTILIZATION = 0.95  # 🚀 MAXIMUM: 95% GPU memory usage
    NUM_GPU = 1                  # 🚀 Single GPU optimization
    ...
    # File paths - Ollama Sequential optimized
    LOG_DIR = Path(__file__).resolve().parent / "logs"
    RESULTS_DIR = Path(__file__).resolve().parent / "results/middle/mistral_7b/with_geom_time/"
        ...
        for attempt in range(1, Config.MAX_RETRIES_PER_REQUEST + 1):
            ...
            try:
                ...
                payload = {
                    ...
                    "options": {
                        # Context window - aligned with Config
                        "num_ctx": Config.NUM_CTX,

                        # Generation parameters - using Config values
                        ...
                        # 🚀 MAXIMUM GPU ACCELERATION - using Config values
                        "num_thread": Config.NUM_THREAD,
                        "num_batch": Config.NUM_BATCH,
                        "num_gpu": Config.NUM_GPU,           # Using Config GPU setting
                        "main_gpu": Config.MAIN_GPU,         # Primary GPU from Config
                        "num_gpu_layers": Config.GPU_LAYERS, # 🚀 CRITICAL: All layers on GPU
            ...
            try:
                for j, prompt in enumerate(preload_prompts):
                    ...
                    payload = {
                        ...
                        "stream": False,
                        "options": {
                            # 🚀 MAXIMUM GPU ACCELERATION - aligned with main payload
                            "num_ctx": Config.NUM_CTX,           # 🚀 Using Config context
                            "num_predict": 128,                  # 🔧 ADEQUATE: Space for JSON
                            "num_thread": Config.NUM_THREAD,     # 🚀 Maximum threads
                            "num_batch": Config.NUM_BATCH,       # 🚀 Maximum batch size
                            "num_gpu_layers": Config.GPU_LAYERS, # 🚀 All layers on GPU
                            "cache_type_k": "f16",
                            "temperature": 0.7
                        }
                    }
...
class PromptBuilder:
    ...
    def create_prompt(
        ...
        pois_df: pd.DataFrame,
        card_id: str,
        top_k: int = Config.POI_PREDICTIONS,
...
class VisitFileProcessor:
    ...
    def _process_cards_parallel(
        ...
        # 🔧 SEQUENTIAL EXECUTION: Force sequential processing with Mistral 7B
        optimal_workers = 1  # ✅ SEQUENTIAL: Only 1 worker for sequential execution

        logger.info(f"🔧 MISTRAL SEQUENTIAL MODE: {optimal_workers} worker for {n_healthy_hosts} healthy hosts")
        ...
        logger.info("🚀 Starting MAXIMUM GPU sequential processing with Mistral 7B (All layers on GPU)...")
        
        # 🔧 SEQUENTIAL PROCESSING: Process cards one at a time
        total_cards = len(cards_to_process)
        processed_count = 0

        logger.info(f"Processing {total_cards} cards sequentially with Mistral 7B (MAXIMUM GPU utilization)")
        
        # 🔧 SEQUENTIAL PROCESSING: Process cards one by one with Mistral 7B
        logger.info("🔧 SEQUENTIAL PROCESSING: Processing cards one by one for maximum stability with Mistral 7B")

        # Global progress bar for sequential processing
        with tqdm(
            total=total_cards,
            desc="Processing cards sequentially (Mistral 7B)",
            unit="card"
        ) as pbar:

            # Process cards sequentially with Mistral 7B
            for i, card_id in enumerate(cards_to_process, 1):
                logger.debug(f"🔧 Processing card {i}/{total_cards}: {card_id} (Mistral 7B sequential)")

                try:
                    # Process card directly (sequential execution)
                    result = card_processor.process_card(card_id)
                    processed_count += 1

                    if result and result.get('status') == 'fatal_error':
                        logger.warning(f"Fatal error for card {card_id}")

                    # Check circuit breaker - reduced pause to prevent blocking
                    if stats.consecutive_failures >= Config.CIRCUIT_BREAKER_THRESHOLD:
                        logger.warning("Too many consecutive failures - brief pause")
                        time.sleep(5)  # 🔧 OPTIMIZED: 5 seconds pause
                        ...
                        if stats.consecutive_failures >= Config.CIRCUIT_BREAKER_THRESHOLD * 2:
                            logger.error("Critical failure threshold reached - saving progress and terminating")
                            try:
                ...
                if processed_count % Config.BATCH_SAVE_INTERVAL == 0:
                    try:
                        card_processor.results_manager.save_batch()
                        logger.info(f"✅ Checkpoint saved after {processed_count} cards (Mistral 7B sequential)")
                    except Exception as save_error:
                        logger.error(f"Failed to save checkpoint: {save_error}")

                # Force garbage collection periodically
                if processed_count % (Config.BATCH_SAVE_INTERVAL * 2) == 0:
                    gc.collect()
                    logger.debug(f"Memory cleanup after {processed_count} cards")

                # Brief pause between cards for stability (optional)
                if i < total_cards:
                    time.sleep(0.1)  # Small pause between cards for stability

        logger.info(f"✅ SEQUENTIAL PROCESSING COMPLETED - {processed_count}/{total_cards} cards processed with Mistral 7B")
